...
namespace Shop.UI.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        [HttpPost("stocks")]
        public async Task<IActionResult> CreateStock([FromBody]CreateStock.Request req) => Ok((await new CreateStock(_ctx).Do(req)));

        [HttpDelete("stocks/{id}")]
        public async Task<IActionResult> DeleteStock(int id) => Ok(await new DeleteStock(_ctx).Do(id));

        [HttpPut("stocks")]
        public async Task<IActionResult> UpdateStock([FromBody]UpdateStock.Request req) => Ok((await new UpdateStock(_ctx).Do(req)));
    }
}
